...
> This project is a specialized **Claude Code Agent Edition** adapted from the original [SakanaAI AI-Scientist-v2](https://github.com/SakanaAI/AI-Scientist-ICLR2025-Workshop-Experiment). It completely restructures the execution model to leverage the `claude` CLI and Anthropic's interactive agent workflows.

Fully autonomous scientific research systems are becoming increasingly capable, with AI playing a pivotal role in transforming how scientific discoveries are made. We are excited to present the Claude Code Agent Edition of The AI Scientist-v2.

This system autonomously generates hypotheses, runs experiments, analyzes data, and writes scientific manuscripts. While the original v2 relied on static python scripts for orchestration, **this edition uses Claude Code's native `.claude/skills/` and hooks** to orchestrate the Best-First Tree Search (BFTS) experiment manager.

> **Caution!**
> This codebase executes Large Language Model (LLM)-written code. There are various risks and challenges associated with this autonomy, including the potential use of dangerous packages, uncontrolled web access, and the possibility of spawning unintended processes. Our added Hooks provide some monitoring, but please ensure you run this within a controlled sandbox environment (e.g., a Docker container).
...
3.  [Manual Execution (Legacy)](#manual-execution-legacy)
...
We have successfully migrated the AI-Scientist-v2 to a Claude Code agent-based structure! You can now run end-to-end scientific research by interacting dynamically with the `claude` CLI.

1. **Install Claude Code:** `npm install -g @anthropic-ai/claude-code`
2. **Setup Codex:** (Optional for adversarial cross-model review) `claude mcp add codex -s user -- codex mcp-server`
3. **Run Pipeline:**
   ```bash
   claude
...
Or you can invoke individual workflows:
...
If you prefer to bypass the Claude Code agent orchestration and use the raw python scripts provided by the v2 architecture:

**1. Ideation:**
```bash
python scripts/run_ideation.py \
 --topic-file "ai_scientist/ideas/my_research_topic.md"
```

**2. BFTS Search:**
```bash
python scripts/run_tree_search.py \
 --load_ideas "ai_scientist/ideas/my_research_topic.json" \
 --load_code \
 --add_dataset_ref
```
...
```
*(These manual processes are primarily kept intact as the backend scripts that the `/skills` now wrap securely.)*
